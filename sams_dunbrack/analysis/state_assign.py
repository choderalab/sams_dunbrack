...
(key_res, dihedrals, distances) = featurize(chain='A', coord='dcd', feature='conf', pdb='5UG9')
...
# state assignment for each frame in the trajectory
print("Assigning states to each frame in a trajectory.")
state_assign = list() # the final state assignment, 1 x n_frame array
for frame in range(len(dih[0])): # for each frame in the trajectory
    # keep track of the feature overlap of each state
    min_dist = 0
    min_state = 0
    for state in range(nstate):
        dist = 0 # the feature overlap (int between 15*0 and 15*1) of a frame to one Dunbrack cluster
        for h in range(ndih):
            #dist += ((dih[h][frame]-dunbrack_phi0[state,h])/dunbrack_dphi[state,h])**2
            dist += (dih[h][frame]-dunbrack_phi0[state,h])**2
        for s in range(ndis):
            #dist += ((dis[s][frame]-dunbrack_r0[state,s])/dunbrack_dr[state,s])**2
            dist += (dis[s][frame]-dunbrack_r0[state,s])**2
        #print("state "+str(state)+": ", dist)
        if state == 0:
            min_dist = dist
        elif min_dist > dist:
            min_dist = dist
            min_state = state
    state_assign.append(min_state)
