...
def compute_simple_protein_features(pdbid, chainid, coordfile, numbering):
    ...
    if coordfile == 'raw_pdb':
        ...
        with urllib.request.urlopen(f'http://www.pdb.org/pdb/files/{pdbid}.pdb') as response:
            ...
            for i in range(len(check)):
                if check[i] == 'ATOM':
                    ...
                    if check[i+4] in list(string.ascii_uppercase):
                        if f'HET{check[i+4]}' not in chain_lst:
                            chain_lst.append(f'HET{check[i+4]}')
                elif check[i] == 'CONECT':
                    break
        with tempfile.TemporaryDirectory() as pdb_directory:
            pdb = os.path.join(pdb_directory, f'{pdbid}_minimized.pdb')
        ...
        pdb = os.path.join(f'{pdbid}_minimized.pdb')
    ...
    dih[0][0] = topology.select(f"chainid {chain_index} and residue {numbering[20]} and name CA")
    dih[0][1] = topology.select(f"chainid {chain_index} and residue {numbering[28]} and name CA")
    dih[0][2] = topology.select(f"chainid {chain_index} and residue {numbering[60]} and name CA")
    dih[0][3] = topology.select(f"chainid {chain_index} and residue {numbering[62]} and name CA")

    # dihedral 1 & 2: X-DFG Phi & Psi
    dih[1][0] = topology.select(f"chainid {chain_index} and residue {numbering[78]} and name C")
    dih[1][1] = topology.select(f"chainid {chain_index} and residue {numbering[79]} and name N")
    dih[1][2] = topology.select(f"chainid {chain_index} and residue {numbering[79]} and name CA")
    dih[1][3] = topology.select(f"chainid {chain_index} and residue {numbering[79]} and name C")

    dih[2][0] = dih[1][1]
    dih[2][1] = dih[1][2]
    dih[2][2] = dih[1][3]
    dih[2][3] = topology.select(f"chainid {chain_index} and residue {numbering[80]} and name N")

    # dihedral 3 & 4: DFG-Asp Phi & Psi
    dih[3][0] = dih[1][3]
    dih[3][1] = dih[2][3]
    dih[3][2] = topology.select(f"chainid {chain_index} and residue {numbering[80]} and name CA")
    dih[3][3] = topology.select(f"chainid {chain_index} and residue {numbering[80]} and name C")

    dih[4][0] = dih[3][1]
    dih[4][1] = dih[3][2]
    dih[4][2] = dih[3][3]
    dih[4][3] = topology.select(f"chainid {chain_index} and residue {numbering[81]} and name N")

    # dihedral 5 & 6: DFG-Asp Chi1 & Chi2
    dih[5][0] = dih[2][3] # DFG-Asp N
    dih[5][1] = dih[3][2] # DFG-Asp CA
    dih[5][2] = topology.select(f"chainid {chain_index} and residue {numbering[80]} and name CB") # DFG-Asp CB
    dih[5][3] = topology.select(f"chainid {chain_index} and residue {numbering[80]} and name CG") # DFG-Asp CG

    dih[6][0] = dih[5][1]
    dih[6][1] = dih[5][2]
    dih[6][2] = dih[5][3]
    dih[6][3] = topology.select(f"chainid {chain_index} and residue {numbering[80]} and name OD1") #DFG-Asp OD1

    # dihedral 7 & 8: DFG-Phe Phi & Psi
    dih[7][0] = dih[3][3]
    dih[7][1] = dih[4][3]
    dih[7][2] = topology.select(f"chainid {chain_index} and residue {numbering[81]} and name CA")
    dih[7][3] = topology.select(f"chainid {chain_index} and residue {numbering[81]} and name C")
    
    dih[8][0] = dih[7][1]
    dih[8][1] = dih[7][2]
    dih[8][2] = dih[7][3]
    dih[8][3] = topology.select(f"chainid {chain_index} and residue {numbering[82]} and name N")

    # dihedral 9: DFG-Phe Chi1
    dih[9][0] = dih[4][3] #DFG-Phe N
    dih[9][1] = dih[7][2] #DFG-Phe CA
    dih[9][2] = topology.select(f"chainid {chain_index} and residue {numbering[81]} and name CB") # DFG-Phe CB
    dih[9][3] = topology.select(f"chainid {chain_index} and residue {numbering[81]} and name CG") # DFG-Phe CG

    # distance 0 & 1: K-E salt bridge
    dis[0][0] = topology.select(f"chainid {chain_index} and residue {numbering[16]} and name NZ")
    dis[0][1] = topology.select(f"chainid {chain_index} and residue {numbering[23]} and name OE1")
    dis[1][0] = dis[0][0]
    dis[1][1] = topology.select(f"chainid {chain_index} and residue {numbering[23]} and name OE2")

    # distance 2 & 3: DFG conformation-related distances
    dis[2][0] = topology.select(f"chainid {chain_index} and residue {numbering[27]} and name CA")
    dis[2][1] = topology.select(f"chainid {chain_index} and residue {numbering[81]} and name CZ")
    dis[3][0] = topology.select(f"chainid {chain_index} and residue {numbering[16]} and name CA")
    dis[3][1] = dis[2][1]

    # distance 4: FRET distance
    dis[4][0] = topology.select(f"chainid {chain_index} and residue {numbering[16]+120} and name CA")
    dis[4][1] = topology.select(f"chainid {chain_index} and residue {numbering[16]+61} and name CA")
